<template>
    <div class='layout flex h-screen'>
        ...
        <div v-if='getSetting.mode === "vertical" || getMenubar.isPhone'
            class='layout-sidebar flex flex-col h-screen transition-width duration-200 shadow' :class='{
            ...
            <div class='layout-sidebar-logo flex h-12 relative flex-center shadow-lg'>
                <img class='h-8' :src='icon'>
                <span v-if='getMenubar.status === 0 || getMenubar.status === 2' class='pl-2'>{{ logoName }}</span>
            </div>
        ...
        <div class='layout-main flex flex-1 flex-col overflow-x-hidden overflow-y-auto'>
            ...
            <div v-if='getSetting.showTags' class='layout-main-tags h-8 leading-8 text-sm text-gray-600 relative z-1'>
                <layout-tags />
            </div>
...
import { logoName,icon } from '/@/config/base'
